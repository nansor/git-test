...
function generatePiece() {
    ...
    var gameBoard = document.getElementById('gameBoard');//返回指定 ID 的元素：

    // 随机获取一种方块
    pieceIdx = Math.floor(PIECE_NUM * Math.random())//floor:返回小于等于x的最大整数://random:返回介于 0（包含） ~ 1（不包含） 之间的一个随机数：
    ...
    // 将这个 piece 挂到 gameBoard 内容板里,改部件的id叫做currentPiece
    newPiece.id = "currentPiece";
    gameBoard.appendChild(newPiece);//添加子元素
...
function setPosition(piece, row, col) {
    ...
    piece.style.top = row * BLOCKSIZE + 'px';//xxx.style.top --> string
    piece.style.left = col * BLOCKSIZE + 'px';//xxx.style.left --> string

    // 设置所有子节点的位置，并储存
    for (var i = 0; i < piece.children.length; i++) {
        block = blockList[i]
        block.rowPos = row + layout[i][0];//设置位置
        block.colPos = col + layout[i][1];
        block.style.top = block.rowPos * BLOCKSIZE + 'px';//储存位置
...
function pieceMoveDown() {
    ...
    */
    ...
    /************************
        YOUR CODE STARTS
    *************************/
    piece = document.getElementById('currentPiece')
    var blockList = piece.children;
...
function fallToBottom(piece) {
   ...
   if (isMoveDownValid(piece)){
       ...
       //确认方块位置
        for (var idx = 0; idx < 4; idx++){
            //blockList[idx].rowPos += maxrownum;//之前已经改变过了方块的rownum，要是再加就会加双倍出方框
            blockList[idx].style.top = blockList[idx].rowPos * BLOCKSIZE + 'px';
       }
       for (var idx = 0; idx < 4; idx++){
        console.log(blockList[idx].rowPos);
   }
   }
    /************************
        YOUR CODE ENDS
...
function isMoveDownValid(piece) {
    ...
    *************************/
    ...
    if (blockList[0].rowPos < 19 && 
        ...
        blockList[3].rowPos < 19 &&//确认所有的方块在旋转之后都能下移而不出边界
